const axios = require('axios');

/**
 * Stalk / cek profil akun Roblox berdasarkan username
 * Source: https://api.azbry.com/api/stalk/roblox?username=
 * @param {string} username
 */
async function stalkRoblox(username) {
  const apiUrl = `https://api.azbry.com/api/stalk/roblox?username=${encodeURIComponent(username)}`;

  let response;
  try {
    response = await axios.get(apiUrl, {
      timeout: 20000,
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0 Safari/537.36',
        'Accept': 'application/json'
      },
      validateStatus: () => true // biar bisa handle status non-200 secara manual
    });
  } catch (err) {
    // Error jaringan (timeout, DNS, koneksi putus, dll)
    throw new Error(`Gagal menghubungi server Roblox stalker: ${err.message}`);
  }

  const data = response.data;

  // Kalau upstream balikin HTML / bukan JSON (biasanya tanda down / cloudflare block)
  if (!data || typeof data !== 'object') {
    throw new Error('Respon dari server tidak valid (bukan JSON). Server mungkin sedang down.');
  }

  if (response.status !== 200 || data.status === false) {
    throw new Error(data.error || data.message || `Username "${username}" tidak ditemukan atau terjadi kesalahan pada server`);
  }

  if (!data.result) {
    throw new Error('Data hasil tidak ditemukan pada respon server');
  }

  const r = data.result;

  return {
    id: r.id,
    username: r.username,
    displayName: r.displayName,
    created: r.created,
    isBanned: r.isBanned,
    presence: {
      status: r.presence?.status ?? null,
      lastOnline: r.presence?.lastOnline ?? null
    },
    social: {
      friends: r.social?.friends ?? 0,
      followers: r.social?.followers ?? 0,
      following: r.social?.following ?? 0
    },
    avatar: r.avatar,
    profileUrl: r.profileUrl
  };
}

module.exports = [
  {
    name: "Stalk Roblox",
    desc: "Cek / stalk profil akun Roblox berdasarkan username",
    category: "Stalker",
    path: "/stalk/roblox?apikey=&username=",
    async run(req, res) {
      const { apikey, username } = req.query;

      if (!apikey || !global.apikey.includes(apikey)) {
        return res.status(403).json({
          status: false,
          error: "Apikey invalid"
        });
      }

      if (!username || !username.trim()) {
        return res.status(400).json({
          status: false,
          error: "Username is required"
        });
      }

      try {
        const result = await stalkRoblox(username.trim());
        return res.status(200).json({
          status: true,
          result
        });
      } catch (error) {
        return res.status(500).json({
          status: false,
          error: error.message || "Terjadi kesalahan pada server"
        });
      }
    }
  }
];
