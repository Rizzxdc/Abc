 ...
 * githubDb.js
 * ------------------------------------------------------------------
 * "Database" sederhana berbasis GitHub: menyimpan array data (users)
 * sebagai satu file JSON di sebuah repository GitHub, lewat GitHub
 * Contents API (https://docs.github.com/en/rest/repos/contents).
 *
 * Butuh environment variable:
 ...
 *   GITHUB_DB_PATH    - (opsional) default: "database/users.json"
 * ------------------------------------------------------------------
 */
...
function getConfig() {
  // .trim() jaga-jaga kalau ada spasi nyangkut pas copy-paste env var di
  // dashboard hosting (penyebab paling umum error 404 padahal repo sudah benar)
  ...
  return {
    ...
    GITHUB_OWNER: trim(GITHUB_OWNER),
    GITHUB_REPO: trim(GITHUB_REPO),
    GITHUB_BRANCH: trim(GITHUB_BRANCH),
    GITHUB_DB_PATH: trim(GITHUB_DB_PATH)
  };
}
